    ...
    <>
      ...
      <div className="loro-wrap" style={{ padding: '56px 0 80px', maxWidth: 560 }}>
        ...
        <p style={{ fontSize: 15, color: 'var(--ink4)', lineHeight: 1.75, marginBottom: 36 }}>
          The signal before the story. Real-time regulatory intelligence for people who need to know what's happening at a payments company before it's in the news.
        </p>

        {/* What you get — B2B */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 1, background: 'var(--border)', marginBottom: 32 }}>
          {[
            { title: 'Loro Score alerts', desc: 'Notified immediately when a monitored entity moves 10+ points. The PayPal cluster triggered at 10.0/10 — 48 hours before any publication covered it.' },
            { title: 'PDMR & insider intelligence', desc: 'Director transactions, PSC changes, share allotments. Parsed to extract person, role, transaction type and value. The signal institutional investors track.' },
            { title: 'Cross-jurisdictional patterns', desc: 'UK FCA + EU + US SEC signals for the same entity within 72 hours. The combination that scores maximum anomaly — and precedes material announcements.' },
            { title: 'Entity pages + score history', desc: 'Full sub-score breakdown, 30-day trend, evidence data. Not just the number — the story behind it.' },
            { title: 'Breaking story access', desc: 'Loro Intelligence subscribers read stories before the weekly newsletter. The editorial loop runs newsroom → Intelligence → Weekly.' },
          ].map(item => (
          ...
          <form action="/api/subscribe" method="POST" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div>
              ...
              <input type="email" name="email" required placeholder="you@firm.com"
                style={{ width: '100%', padding: '11px 14px', border: '1px solid #2A2A2A', fontSize: 14, color: '#F5F5F4', background: '#141414', outline: 'none', fontFamily: "'Inter', sans-serif", boxSizing: 'border-box' }} />
            </div>
            ...
            <div>
              ...
              <input type="text" name="organisation" required placeholder="Fund, firm or company"
                style={{ width: '100%', padding: '11px 14px', border: '1px solid #2A2A2A', fontSize: 14, color: '#F5F5F4', background: '#141414', outline: 'none', fontFamily: "'Inter', sans-serif", boxSizing: 'border-box' }} />
            </div>

            <div>
              <label style={{ fontSize: 11, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#3A3A3A', display: 'block', marginBottom: 6 }}>
                Role / how you'd use this
              </label>
              <select name="role"
                style={{ width: '100%', padding: '11px 14px', border: '1px solid #2A2A2A', fontSize: 14, color: '#F5F5F4', background: '#141414', outline: 'none', fontFamily: "'Inter', sans-serif", boxSizing: 'border-box' }}>
            ...
            <div style={{ fontSize: 11, color: '#3A3A3A' }}>
              We'll respond within 48 hours. Early access pricing available.
            </div>
          </form>
        </div>
